...
class Transformer:
    def __init__(self,hp):
        self.hp = hp
        self.idx2token, self.token2ind = load_vocab(hp.idx2token_path,hp.token2idx_path)

        self.embeddings = self.get_token_embeddings(hp.vocab_size,hp.num_units)

        self.xs = tf.placeholder(tf.int32,[None,hp.maxlen],name='xs')
        self.decode_inputs = tf.placeholder(tf.int32,[None,None],name='decode_inputs')
        self.ys = tf.placeholder(tf.int32,[None,hp.maxlen - 1], name='ys')
    ...
    def multi_attention(self,queries,keys,values,
                        num_heads=8,training=True,causality=False,scope='multi_attention'):
        ...
        with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
            ...
            outputs = self.scaled_dot_project_attention(Q_,K_,V_,
                                                        training=training,causality=causality)
    ...
    def scaled_dot_project_attention(self,Q,K,V
                                     ,training=True,causality=False,scope='scaled_dot_project_attention'):
        ...
        with tf.variable_scope(scope,reuse=tf.AUTO_REUSE):
            ...
            outputs = tf.layers.dropout(outputs,rate=self.dropout_rate,training=training)
    ...
    def noam_scheme(self,lr,global_step,warmup=4000.):
        ...
        return lr * warmup ** 0.5 * tf.minimum(step * warmup ** -1.5, step ** -0.5)
    ...
    def encoder(self,training=True):
    ...
    def decoder(self,memory,training=True):
