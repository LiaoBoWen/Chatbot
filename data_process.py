...
def save_data_to_pickle(data,data_path='./data/data'):
    ...
    train_decode_inputs = decode_inputs[:train_size]
    test_decode_inputs = decode_inputs[train_size:]
    train_ys = ys[:train_size]
    test_ys = ys[train_size:]

    data = ((train_xs,train_decode_inputs,train_ys),(test_xs,test_decode_inputs,test_ys))
...
def get_batch(data,epoch=50,batch_size=32,shuffle=True):
    ...
    for epoch_i in range(epoch):
        ...
        for i in range(batch_num):
            ...
            yield xs[start:end], decode_inputs[start:end], ys[start:end], epoch_i
...
if __name__ == '__main__':
    ...
    for xs, decode_inputs, ys, epoch_i in get_batch(train_data,2,4,shuffle=False):
        print('xs:')
        print(xs)
        print('decode_inputs:')
        print(decode_inputs)
